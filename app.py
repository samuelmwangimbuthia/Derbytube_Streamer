...
def pad_added_handler(src, new_pad, data):
    video_convert_element, audio_convert_element = data
    ...
    if new_pad_type.startswith("video/x-raw"):
        sink_pad = video_convert_element.get_static_pad("sink")  # get the sink pad of the convert element
        ...
        sink_pad = audio_convert_element.get_static_pad("sink")
        link_result = new_pad.link(sink_pad)
        logger.debug(f"linking new pad {new_pad.get_name()} of type {new_pad_type} ")
         #attempt to link the new pad to the convert element's sink pad
        if link_result != Gst.PadLinkReturn.OK:
...
def main():
    ...
    if not pipeline or not source or not video_sink or not audio_sink or not video_convert or not resample or not audio_convert :
    ...
    if not audio_convert.link(resample):
        ...
        sys.exit(1)
    if not resample.link(audio_sink):
        logger.error("Failed to link resample to sink.")
        sys.exit(1)
    ...
    source.connect("pad-added", pad_added_handler, (video_convert, audio_convert))#convert is the element we want to link to the newly created pad from the source element.
